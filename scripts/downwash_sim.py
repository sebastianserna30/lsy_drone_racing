...
avoiding the downwash zones of 5 hovering obstacle drones rendered
as full drone models. A translucent cone is drawn below each obstacle
drone to visualise its downwash region.
...
def obstacle_positions(t: float) -> np.ndarray:
    """Return (5, 3) float32 array of obstacle drone positions at time t."""
    y1 =  _OPP_AMP * np.sin(2 * np.pi * _OPP_FREQ * t)
    y2 = -_OPP_AMP * np.sin(2 * np.pi * _OPP_FREQ * t)
    dynamic = np.array([[-0.5, y1, 1.0], [0.5, y2, 1.0]], dtype=np.float32)
...
def draw_downwash_cone(sim: Sim, apex: np.ndarray) -> None:
    ...
    Geom budget: 2 rings × 12 segs + 6 capsule spokes = 30 per cone × 5 = 150 total.
    ...
    for ring_i in range(1, n_rings + 1):
        ...
        ring = np.column_stack([
            apex[0] + r * np.cos(angles),
            apex[1] + r * np.sin(angles),
            np.full(n_ring_pts + 1, z),
        ]).astype(np.float32)
...
class DownwashTestController(AttitudeMPPIController):
    """MPPI with straight-line reference and 5-slot opponent_pos."""
    ...
    def create_waypoints(self, obs: dict) -> None:
        start = obs["pos"].copy()
        goal = DRONE_GOAL.copy()
        mid = ((start + goal) / 2).astype(np.float32)
        self._waypoints = np.array([start, mid, goal])

    def create_spline(self) -> None:
        self._t_total = 15.0
        t = np.linspace(0.0, self._t_total, len(self._waypoints))
    ...
    def render_callback(self, sim: Sim) -> None:
        ...
        Geom budget breakdown (maxgeom=1000):
          ...
          5 × cones         150 geoms (drawn in simulate())
          Total             ~277
        """
...
def set_drone_state(
    ...
    sim.data = sim.data.replace(
        states=sim.data.states.replace(pos=p, quat=q, vel=v, ang_vel=av)
    )
...
def simulate(render: bool = False) -> None:
    ...
    n_drones_total = 1 + DownwashTestController.N_OPPONENTS  # 6
    sim = Sim(
        n_worlds=1,
    ...
    init_pos  = np.vstack([DRONE_START[None], opp0])               # (6, 3)
    init_quat = np.tile([0., 0., 0., 1.], (n_drones_total, 1))     # (6, 4)
    ...
    # Obstacle drone hover commands (drones 1-5 just hover)
    hover_cmds = jnp.tile(HOVER_CMD, (1, n_drones_total, 1))  # (1, 6, 4)
    sim.data = sim.data.replace(
    ...
    cam_cfg = {"distance": 6.0, "azimuth": 150.0, "elevation": -20.0,
               "lookat": [0.0, 0.0, 0.8]}
    fps = 60
    t = 0.0
    i = 0
    ...
    while i < MAX_STEPS:
        opp_pos = obstacle_positions(t)   # (5, 3)

        # Inject opponent positions into controller before computing action
        controller.opponent_pos = jax.device_put(
            jnp.array(opp_pos), controller.sim.device
        )
        ...
        # Override ONLY obstacle drone states (indices 1-5); never touch drone-0.
    ...
    print(f"Steps in any downwash zone:       {downwash_steps}  ({downwash_steps / CTRL_FREQ:.1f} s)")
