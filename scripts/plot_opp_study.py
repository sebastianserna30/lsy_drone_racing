...
def _latest(root: Path) -> Path | None:
    ...
    found = [p for p in root.glob("sweep_opp_progress_*.csv") if not p.name.endswith("_runs.csv")]
...
def _read(path: Path) -> list[dict]:
    with path.open() as fh:
        return [
            {k: float(v) if v not in ("", "nan") else float("nan") for k, v in row.items()}
...
def _solo_pace(root: Path) -> dict[float, float]:
    ...
    if not path.exists():
        return {}
    with path.open() as fh:
        return {float(r["progress"]): float(r["solo_lap_time"]) for r in csv_mod.DictReader(fh)}


def plot(csv: str | None = None, out: str = "opp_study.png") -> int:
    ...
    if path is None or not path.exists():
        print("no sweep_opp_progress_*.csv found -- has the sweep written its first seed yet?")
        return 1
    rows = _read(path)
    if not rows:
    ...
    fig.suptitle(f"Opponent pace sweep - ego pinned at progress 1.5, n = {n} seeds", y=1.0)
    fig.tight_layout()
